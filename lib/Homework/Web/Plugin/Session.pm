package Homework::Web::Plugin::Session;
use strict;
use warnings;
use utf8;
...
my $cipher = Crypt::CBC->new({
    key => 'do15fC06JlA6x3UnMcfj8FroWcLOdHyf',
    cipher => 'Rijndael',
});
sub _session {
    my $self = shift;
    ...
    if (!exists $self->{session}) {
        $self->{session} = HTTP::Session2::ClientStore2->new(
            env => $self->req->env,
            secret => 'nub3yAkopHZ9i51JQ0r50jswg5kLrR0x',
            cipher => $cipher,
        );
    }
    return $self->{session};
...
This module manages session for Homework.
